...
C1=2*h_conv1*dy/k;C2=2*h_conv2*dy/k;C3=(q*alfa*dt)/k;
...
for i=1:nx-1
    for j=1:ny
        if i==1
            if j==1
                B(i,j)=-2*C3-2*Sx*IT(i,j)+2*Sy*T_inf1*C1/(C1-1)-Sx*IT(i+2,j)+...
                    (2*Sx-1+2*Sy)*IT(i+1,j)-Sy*(1+1/(C1-1))*IT(i+1,j+1);  %Top Left Edge
            elseif j==ny
                B(i,j)=-2*C3-2*Sy*C2*T_inf2/(C2+1)-Sx*IT(i+2,j)+...
                    (2*Sx-1+2*Sy)*IT(i+1,j)-2*Sx*IT(i,j)-Sy*(1/(C2+1)+1)*IT(i+1,j-1);  %Bottom left edge
            else
                B(i,j)=-2*C3-Sx*IT(i+2,j)+(2*Sx-1+2*Sy)*IT(i+1,j)-...
                    2*Sx*IT(i,j)-Sy*IT(i+1,j+1)-Sy*IT(i+1,j-1); %first row interior nodes
            end
        elseif i==nx-1
            if j==1
                B(i,j)=-2*C3-2*Sx*IT(i,j)+2*Sy*T_inf1*C1/(C1-1)+...
                    (2*Sx-1+2*Sy)*IT(i+1,j)-Sy*(1/(C1-1)+1)*IT(i+1,j+1);  %Bottom Left Edge
            elseif j==ny
                B(i,j)=-2*C3-2*Sy*C2*T_inf2/(C2+1)-2*Sx*IT(i,j)+(2*Sx-1+2*Sy)*...
                    IT(i+1,j)-Sy*(1/(C2+1)+1)*IT(i+1,j-1);                %Bottom right edge
            else
                B(i,j)=-2*C3+(2*Sx-1+2*Sy)*IT(i+1,j)-2*Sx*IT(i,j)-Sy*IT(i+1,j+1)-Sy*IT(i+1,j-1); %interior nodes of 
            end
        else
            if j==1
                B(i,j)=-2*C3-Sx*IT(i,j)+2*Sy*T_inf1*C1/(C1-1)-Sx*IT(i+2,j)+...
                    (2*Sx-1+2*Sy)*IT(i+1,j)-Sy*(1+1/(C1-1))*IT(i+1,j+1);  %Top Edge
            elseif j==ny
                B(i,j)=-2*C3-2*Sy*C2*T_inf2/(C2+1)-Sx*IT(i-1,j)+...
                    (2*Sx-1+2*Sy)*IT(i+1,j)-Sx*IT(i,j)-Sy*(1/(C2+1)+1)*IT(i+1,j-1);  %Bottom Edge
            else
                B(i,j)=-2*C3-Sx*IT(i+2,j)+(2*Sx-1+2*Sy)*IT(i+1,j)-...
                    Sx*IT(i,j)-Sy*IT(i+1,j+1)-Sy*IT(i+1,j-1);  %Interior nodes
            end
        end
    end
end
b=reshape(B',[],1);
Q=Seidel(A,b,1);   %Using gauss seidel with relaxation factor
IT(1,1:ny)=100;
for i=2:nx
    for j=1:ny
        IT(i,j)=Q(j+(i-2)*ny,1);
