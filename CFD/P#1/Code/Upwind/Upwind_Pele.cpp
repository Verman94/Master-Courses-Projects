...
//  Domain of x:  0 <= x <= 1
//  Initial condition:      u(x,0)=1.0           x <= 0.1 
//                          u(x,0)=0.0    0.1 <= x <= 0.3
//                          u(x,0)=1.0           x >= 0.3 
...
int main ()
{
	...
	/*----------------------------------------*/
	//SOLVING ADVECTION EQUATION USING FIRST ORDER UPWIND METHOD
	...
	for (int i=0 ; i <= n-2 ; i++)
	{
	    for (int j=0 ; j <= IM-1 ; j++)
		{
			...
			if (j == 0)
			{
				...
				//PERIODIC BOUNDRAY CONDITION { u [i][j-1] = u [i][IM-2] }

				u [i+1][j] = u [i][j] - co * (u [i][j] - u [i][IM-2]);
			}

			else

			{
				u [i+1][j] = u [i][j] - co * (u [i][j] - u [i][j-1]);
			}
	...
	ofstream plot;
	plot.open("Upwind.txt",ios::out);
	//plot << "TITLE=\"First Order UPWIND METHOD Courant Number is 0.5\"" << endl << "VARIABLES=Distance (m),Velocity (m/s)" << endl;
