...
//  Domain of x:  0 <= x <= 1
//  Initial condition:      u(x,0)=.0           x < 0.1 
//                          u(x,0)=1.0    0.1 <= x < 0.3
//                          u(x,0)=.0           x >= 0.3 
...
int main ()
{
	...
	//SOLVING ADVECTION EQUATION USING LAX METHOD
	...
	for (int i=0 ; i <= n-2 ; i++)
	{
	    for (int j=0 ; j <= IM-1 ; j++)
		{
			...
			if (j == 0)
			{
				u [i+1][j] = (u [i][j+1] + u [i][IM-2]) / 2 - co * (u [i][j+1] - u [i][IM-2]) / 2;
			}

			else if (j == IM-1)

			{
				u [i+1][j] = (u [i][1] + u [i][j-1]) / 2 - co * (u [i][1] - u [i][j-1]) / 2;
			}

			else
			{
				u [i+1][j] = (u [i][j+1] + u [i][j-1]) / 2 - co * (u [i][j+1] - u [i][j-1]) / 2;
			}
