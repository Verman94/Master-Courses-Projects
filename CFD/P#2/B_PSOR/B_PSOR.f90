...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    real,Dimension(:,:),Allocatable  ::  w,vor,m,sy,sye,u,v,p,x,y,px,py,ress
    real                             ::  beta=1.5,error=.001,t=.0,dt=.001,err=.0,d=.0,res=1.0,alpha=1.2
    integer                          ::  i=0,j=0,k=0,it1=0,it2=0,im,jm,il,ir,ju,numstep,mm
    ...
    Allocate (p(im,jm),x(im,jm),y(im,jm),px(im,jm),py(im,jm),ress(im,jm))
      ...
      do mm=1,10
            ...
            do j=2,jm-1
                do i=2,im-1
                    ...
                    Ress(i,j) = (1/(2*(1+Beta)))*( Sy(i+1,j) + Sy(i-1,j) &
                                + Beta*(Sy(i,j+1) + Sy(i,j-1)) + W(i,j)*((Dx)**2) )
                    Sy(i,j) = (1-Alpha)*Sy(i,j) + Alpha*Ress(i,j)
                end do
            end do
            do j=2,ju
                do i=il,ir
...
!	Vorticity in Boundary condition
        ...
        do i=2,im-1
            do j=2,jm-1
                w(i,1)=-2.0*sy(i,2)/(dy*dy)					! bottom wall
                w(i,jm)=-2.0*sy(i,jm-1)/(dy*dy)-2.0/dy		! top wall
        ...
        do i=il+1,ir-1
            do j=2,ju-1
                w(i,ju)=-2.0*sy(i,ju+1)/(dy*dy)         	! top wall
                w(il,j)=-2.0*sy(il+1,j)/(dx*dx)					! right wall
                w(ir,j)=-2.0*sy(ir-1,j)/(dx*dx)				! left wall
            end do
		end do
