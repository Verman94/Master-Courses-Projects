...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 PROGRAM power2
    ...
    Real*8,PARAMETER                   ::  T_inf=300.0,RE=1000.0,PR=7.,ERROR=1E-04,q=10000.0
    ...
    REAL*8,Dimension(IM)               ::  T_STAR,TM,X_STAR,HH,NU,HH_M,NU_M
    ...
    Do i=2,im-1
        Do j=2,jm-1
            ...
            ae(i,j) =   De(i,j) * max(0.0,(1-0.1*ABS(Fe(i,j)/De(i,j)))**5)  +   max(.0,-Fe(i,j))
            aw(i,j) =   Dw(i,j) * max(0.0,(1-0.1*ABS(Fw(i,j)/Dw(i,j)))**5)  +   max(.0,Fw(i,j))
            an(i,j) =   Dn(i,j) * max(0.0,(1-0.1*ABS(Fn(i,j)/Dn(i,j)))**5)  +   max(.0,-Fn(i,j))
            as(i,j) =   Ds(i,j) * max(0.0,(1-0.1*ABS(Fs(i,j)/Ds(i,j)))**5)  +   max(.0,Fs(i,j))
     ...
     Do i=2,im-1
        Do j=2,jm-1
            ...
            Su(i,jm-1)  =   an(i,jm-1)*q*dy/k
            Sp(i,jm-1)  =   0.0

            !!! Bottom Wall  Constant Wall Temperature
            Su(i,2) =   as(i,2)*q*dy/k
            Sp(i,2) =   0.0
		END DO
     END DO
    ...
    !!!    Left Bottom Corner
    Sp(2,2)    =   -aw(2,2)
    Su(2,2)    =   aw(2,2)*T_inf + as(2,2)*q*dy/k

    !!!    Left Top Corner
    Sp(2,jm-1) =   -aw(2,jm-1)
    Su(2,jm-1) =   aw(2,jm-1)*T_inf + an(2,jm-1)*q*dy/k

    !!!    Right Bottom Corner
    Sp(im-1,2) =   0.0
    Su(im-1,2) =   as(im-1,2)*q*dy/k

    !!!    Right Top Corner
    Sp(im-1,jm-1)= 0.0
    Su(im-1,jm-1)= an(im-1,jm-1)*q*dy/k
